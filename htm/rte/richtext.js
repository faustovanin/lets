...
function getRichText(rte, content, width, height, buttons, readOnly) {
	if (isRichText) {
		if (allRTEs.length > 0) allRTEs += ";";
		allRTEs += rte;
	...
	//		html = '		<td><img class="rteImage" src="' + imagesPath + 'redo.gif" width="25" height="24" alt="Redo" title="Redo" onClick="rteCommand(\'' + rte + '\', \'redo\')"></td>');
		...
		html += '<iframe id="' + rte + '" name="' + rte + '" width="' + width + 'px" height="' + height + 'px" src="' + includesPath + 'blank.htm"></iframe>';
		...
		//document.getElementById('hdnrte1').value = address;
		//enableDesignMode('rte1', 'htm/rte', true);
		enableDesignMode(rte, content, readOnly);
	} else {
		if (!readOnly) {
			html += '<textarea name="' + rte + '" id="' + rte + '" style="width: ' + width + 'px; height: ' + height + 'px;">' + content + '</textarea>';
		} else {
			html += '<textarea name="' + rte + '" id="' + rte + '" style="width: ' + width + 'px; height: ' + height + 'px;" readonly>' + content + '</textarea>';
		}
	}
	return html;
}
